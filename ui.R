...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        column(12,
               selectInput("population", label = h3("population"), 
                           choices = list("abundant"="a","clustered"="c","numerous"="n", "scattered"="s","several"="v","solitary"="y" ),
                           selected = 1))
      )
    ),
    ...
    mainPanel(
      p('This app uses the nnet package to create a 4-layer artificial neural network trained using the UCI Machine Learning Repoository\'s Mushroom Data Set to predict whether a mushroom is poisinous or edible.'),
      h3(textOutput("fit")),
